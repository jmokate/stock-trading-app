...
function Dashboard() {
  ...
  const [userInput, setUserInput] = useState({ticker: "", amount: null})
  ...
  const buyStock = (event) => {
    ...
    const stocks = buy.map(stock =>
      <div><td>{stock.ticker}</td>
      </div>
      )
    const stockAmount = buy.map(stock => <div>
      <td>{stock.amount}</td></div> )
  ...
  return (
    <Container className="user-info">
      ...
      <tbody>
        ...
        <td>{stocks}</td>
        <td>{stockAmount}</td>
        <td><Button variant="success" size="sm">Buy More</Button> <Button variant="danger" size="sm" size="sm">Sell</Button></td>
        </tr>
      </tbody>
    </Table>
